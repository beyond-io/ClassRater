...
class TestSignIn:
    ...
    def test_sign_in_invalid(self, not_signed_up_details):
        user = authenticate(username=not_signed_up_details[0], password=not_signed_up_details[1])
        assert user is None

    def test_sign_in_valid(self, signed_up_details):
        user = authenticate(username=signed_up_details[0], password=signed_up_details[1])
        assert user

    # ------------------------Front-End testing------------------------ #

    def test_post_valid_sign_in_with_client(self, client, signed_up_details):
    ...
    def test_invalid_sign_in_redirect(self, client, not_signed_up_details):
        response = client.post('/users/sign_in/', data={
